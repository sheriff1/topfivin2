#!/usr/bin/env python3
"""
Backfill: SummaryV3 DF7 extras for existing game_stats rows.

Calls BoxScoreSummaryV3 DF7 (team game summary) for every game that has
...
Note: BoxScoreSummaryV3 is already called by fetch_nba_stats.py for DF4
(per-period scores). DF7 was available but never extracted. This script
backfills those columns into already-existing game_stats rows.
"""
...
def fetch_summary_extras(game_id):
    """Return dict of team_id -> {biggest_lead, bench_points, ...} or raise."""
    result = BoxScoreSummaryV3(game_id=game_id, timeout=60)
    df = result.get_data_frames()[7]  # DF7 = team game summary
    out = {}
...
def main():
    ...
    for idx, game_id in enumerate(pending, 1):
        ...
        for attempt in range(RETRY_ATTEMPTS):
            try:
                delay()
                team_stats = fetch_summary_extras(game_id)

                for team_id, stats in team_stats.items():
                    cur.execute("""
                        UPDATE game_stats
                        SET biggest_lead=%(biggest_lead)s,
