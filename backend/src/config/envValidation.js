...
function validateEnvironment() {
  ...
  if (process.env.DB_PORT) {
    ...
    if (isNaN(dbPort) || dbPort < 1 || dbPort > 65535) {
      errors.push(`DB_PORT must be a valid number between 1 and 65535. Got: ${process.env.DB_PORT}`);
    }
  }
  ...
  if (process.env.REDIS_PORT) {
    ...
    if (isNaN(redisPort) || redisPort < 1 || redisPort > 65535) {
      errors.push(`REDIS_PORT must be a valid number between 1 and 65535. Got: ${process.env.REDIS_PORT}`);
    }
  }
